...
- 비고 : token_value는 그 자체로 고유한 기본키이고 사용자 로그인시 생성되고 만료되면 스케쥴러에 따라 삭제되거나 로그아웃시 삭제됩니다. 만료되었지만 삭제되지 않은 세션토큰은 매시간 스케쥴러를 통해 삭제됩니다. 이 테이블을 N대1 관계로 사용자(users)테이블과 연결되어 있습니다. 
