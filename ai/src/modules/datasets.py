...
class CombinedDataset:
    def __init__(
        ...
        self.norm = {"mean": (0.485, 0.456, 0.406), "std": (0.229, 0.224, 0.225)}
    ...
    def process_datasets(self) -> Tuple[Dataset, Dataset]:
        ...
        for root_dir in self.root_dirs:
            ...
            train_dataset = TransformableSubset(
                train_dataset.dataset,
                train_dataset.indices,
                transform=self.transform_train,
            )
            val_dataset = TransformableSubset(
                val_dataset.dataset, val_dataset.indices, transform=self.transform_val
            )
    ...
    @staticmethod
    def split_dataset(
        dataset: Dataset, fold_idx: int, n_folds: int, random_seed: int
    ...
    def print_class_statistics(train_dataset: Subset, val_dataset: Subset):
        ...
        def count_and_ratio(subset: Subset, class_idx: int) -> Tuple[int, float]:
            count = sum(
                1 for idx in subset.indices if subset.dataset.targets[idx] == class_idx
            )
        ...
        for class_idx in class_indices:
            train_class_count, train_class_ratio = count_and_ratio(
...
if __name__ == "__main__":
    ...
    cfg = utils.load_yaml("../config/train.yaml")
    root_dirs = cfg["DATASET"]["TRAIN_DATA_DIR"]

    combined_train_dataset, combined_val_dataset = CombinedDataset(root_dirs)

    print(
        f"Total number of samples in combined train dataset: {len(combined_train_dataset)}"
    )
    print(
        f"Total number of samples in combined validation dataset: {len(combined_val_dataset)}"
    )
