...
class Trainer:
    ...
    def train(self) -> None:
        ...
        early_stopping = (
            EarlyStopping(patience=self.config["TRAIN"]["PATIENCE"], verbose=True)
            if self.config["TRAIN"]["PATIENCE"] > 0
            else None
        )
        ...
        if self.resume:
            ...
            try:
                weight_dir = [
                    ...
                    if type(self.model).__name__ == "_".join(p.split("_")[:-2])
                ]
                ...
                self.model.load_state_dict(
                    torch.load(
                        weight_dir / f"checkpoint_epoch_{st_epoch-1}.pt",
                        weights_only=True,
                    )
                )
    ...
    def _train_epoch(self, epoch: int, early_stopping: Optional[EarlyStopping]) -> bool:
        ...
        pbar = tqdm(
            self.train_loader,
            desc=f"Train Epoch [{epoch+1}/{self.config['TRAIN']['EPOCHS']}]",
        )
        ...
        for data, target in pbar:
            ...
            with autocast(
                enabled=self.config["TRAIN"]["AMP"], device_type=self.device.type
            ):
