...
def download_cataract_detection_using_cnn(dataset_dir):
    clone_git(
        "https://github.com/krishnabojha/Cataract_Detection-using-CNN.git", "tmp/"
    )
...
def download_cataract_detection_and_classification(dataset_dir):
    clone_git(
        "https://github.com/piygot5/Cataract-Detection-and-Classification.git", "tmp/"
    )
...
def download_kaggle_cataract_aksh(dataset_dir):
    download_dir = download_kaggle_dataset(
        "akshayramakrishnan28/cataract-classification-dataset"
    )

    arrange_dataset(
        download_dir, dataset_dir, {"1": ["train/immature", "train/mature"]}
    )
