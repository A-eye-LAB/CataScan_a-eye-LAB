...
def process_single_label_dataset(
    src_path: Path, dst_path: Path, label: str
) -> tuple[int, int]:
    processed_count = 0
    error_count = 0
        ...
        for img_path in class_path.glob("*"):
            if img_path.suffix.lower() in (".png", ".jpg", ".jpeg"):
                try:
                    # 파일 이름에 split 정보 추가
                    new_filename = (
                        f"{split}_{img_path.stem}" if split else img_path.stem
                    )
...
def move_and_label_images(dataset_paths, dataset_type: str) -> None:
    ...
    if isinstance(label_mapping, str):
        processed, errors = process_single_label_dataset(
            src_path, dst_path, label_mapping
        )
    ...
    for split in SPLIT_SETTINGS[dataset_type]:
        ...
        if split:
            logging.info(
                f"{split} 분할 처리된 이미지 수: {split_processed} (에러: {split_errors})"
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default="./data",
        help="Path to download datasets (default: ./data)",
    )
